...
chrome.notifications.getPermissionLevel((level) => {
	console.log('perm level', level);
});
setTimeout(() => {
	/*var opt = {
		type: 'image',
		title: 'Test notification!',
		message: 'This is a test noti, how is it workin?',
		iconUrl: '/resources/images/elixr-light-48.png',
		imageUrl: 'https://uploads.mixer.com/cover/189a96c79efbea1f1fa3.jpg'
	};

	chrome.notifications.create('test', opt, () => {
		console.log('DISPLAYED NOTI!');
		console.log('Last error:', chrome.runtime.lastError); 
	});*/

	Notification.requestPermission().then(function(result) {
		console.log(result);
		var img = 'https://uploads.mixer.com/avatar/pad9k5sx-3853195.jpg';
		var text = 'Blocks4Life | ColonySurvival';
		var notification = new Notification('ebiggz has gone live!', { 
			body: text, 
			icon: img,
			badge: img,
			tag: 'Mixer',
			image: 'https://thumbs.mixer.com/channel/315931.small.jpg', 
			silent: true 
		});

		notification.onclick = function(event) {
			event.preventDefault(); // prevent the browser from focusing the Notification's tab
			window.open('https://mixer.com/ebiggz', '_blank');
		};
